import { ExecutionPlan } from '../planning/ExecutionPlan.js';
import { GovernanceContextSnapshot } from './GovernanceContextSnapshot.js';
import { GovernanceRule, RuleResult, RuleAST, RuleStage, RuleExpansion, EventTimelineRecord, DecisionReason } from './GovernanceRule.js';
import { Goal } from '../planning/Goal.js';
import { GovernanceGraphValidator } from './GovernanceGraphValidator.js';
...
export class GovernanceRuleEngine {
  ...
  private resolveFieldValue(field: string, plan: ExecutionPlan, goal: Goal, snapshot: GovernanceContextSnapshot): any {
    ...
    if (field === 'plan.totalCost') {
      return plan.nodes.reduce((acc: number, node) => acc + (node.estimatedCostUsd || 0), 0);
    }
    return undefined;
  }
}
