...
import { EvaluationProvider } from '../plugins/CapabilityProvider.js';
import { PlanningSession, ScoredPlan } from './PlanningSession.js';
import { PolicyResolver } from './PolicyResolver.js';
import { PolicyDecisionRecord } from './EvaluationPolicy.js';
...
export class MetaPlanner implements PlannerProvider {
  ...
  public async createPlan(goal: Goal, context: ExecutionContext): Promise<ExecutionPlan> {
    ...
    const scoredPlans: ScoredPlan[] = await evaluator.evaluatePlans(candidates, goal, context, activePolicy);
    ...
    if (!bestScored) {
      throw new Error('Evaluation produced no scored plans');
    }
    ...
    const session: PlanningSession = {
      ...
      decision: {
        selectedPlanId: bestScored.plan.id,
        needsApproval: activePolicy.tieResolution === 'NeedsApproval' && scoredPlans.length > 1 && bestScored.score.compositeScore === scoredPlans[1]?.score.compositeScore,
