import { EvaluationPolicy, DefaultEvaluationPolicies, CompositeEvaluationPolicy, EvaluationWeights, VetoThresholds } from './EvaluationPolicy.js';
import { Goal } from './Goal.js';
import { ExecutionContext } from '../context/ExecutionContext.js';
import { GovernanceGraphValidator } from '../governance/GovernanceGraphValidator.js';
...
export class PolicyResolver {
  ...
  public static compose(policies: EvaluationPolicy[]): CompositeEvaluationPolicy {
    ...
    let tieRes = first.tieResolution;
    let conflictInTieRes = false;
    let compositeRules: import('../governance/GovernanceRule.js').GovernanceRule[] = [];
    ...
    const sum = (Object.values(compositeWeights) as number[]).reduce((a, b) => a + b, 0);
    if (sum > 0) {
    ...
    if (conflictInTieRes) {
      ...
      provenance.tieResolutionSource = last.name;
    }
  ...
  private static findPolicyById(id: string): EvaluationPolicy | undefined {
    return (Object.values(DefaultEvaluationPolicies) as EvaluationPolicy[]).find(p => p.id === id);
  }
}
