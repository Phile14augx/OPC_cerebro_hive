...
export default function HandbookPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0b0f14] pt-[72px] text-slate-100">
      <div className="fixed inset-x-0 top-[72px] z-40 h-0.5 bg-white/5"><div className="h-full bg-gradient-to-r from-cyan-400 via-violet-400 to-emerald-400 transition-all duration-500" style={{ width: `${progress}%` }} /></div>
      <header className="sticky top-[72px] z-30 border-b border-white/8 bg-[#0b0f14]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1800px] items-center gap-3 px-4 lg:px-7">
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="grid h-9 w-9 place-items-center rounded-lg border border-white/10 text-slate-300 hover:bg-white/5 lg:hidden" aria-label="Toggle handbook navigation"><Menu size={18} /></button>
          ...
          <div className="mx-auto hidden w-full max-w-md md:block"><label className="flex h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 text-slate-400 focus-within:border-cyan-400/50"><Search size={16} /><input value={query} onChange={(e) => setQuery(e.target.value)} className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-slate-500" placeholder="Search the handbook..." /><kbd className="rounded border border-white/10 px-1.5 py-0.5 text-[10px]">⌘ K</kbd></label></div>
          <button onClick={() => setStoryMode(!storyMode)} className="ml-auto rounded-lg border border-white/10 px-3 py-2 text-xs font-semibold text-slate-300 hover:border-cyan-400/40 hover:text-cyan-300">{storyMode ? "Explorer" : "Story mode"}</button>
          <button className="hidden grid h-9 w-9 place-items-center rounded-lg border border-white/10 text-slate-300 hover:bg-white/5 sm:grid" aria-label="More actions"><MoreHorizontal size={18} /></button>
        </div>
      </header>

      <div className="mx-auto grid max-w-[1800px] grid-cols-1 lg:grid-cols-[288px_minmax(0,1fr)_250px]">
        <aside className={cn("fixed inset-y-0 left-0 z-50 w-[288px] overflow-y-auto border-r border-white/8 bg-[#0d1219] px-4 pb-8 pt-24 transition-transform lg:sticky lg:top-[136px] lg:z-10 lg:h-[calc(100vh-136px)] lg:translate-x-0 lg:pt-6", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
        ...
        <main className="min-w-0 px-4 py-7 sm:px-7 lg:px-12 lg:py-12">
          <div className="mx-auto max-w-[820px]">
            {query && <div className="mb-8 rounded-xl border border-white/10 bg-white/[0.03] p-3"><p className="mb-2 text-xs text-slate-500">{results.length} matching chapters</p><div className="flex flex-wrap gap-2">{results.map((result) => <button key={result.id} onClick={() => select(result.id)} className="rounded-lg bg-white/5 px-3 py-1.5 text-xs text-cyan-200 hover:bg-cyan-400/10">{result.title}</button>)}</div></div>}
            <div className="relative mb-10 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-[#111c2a] via-[#111522] to-[#19122b] p-7 shadow-2xl shadow-cyan-950/20 sm:p-10">
              ...
              <div className="relative"><div className="mb-5 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-cyan-300"><Sparkles size={14} /> {active.eyebrow}</div><h1 className="max-w-xl font-space text-4xl font-bold tracking-tight text-white sm:text-5xl">{active.title}</h1><p className="mt-5 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">{active.description}</p><div className="mt-7 flex flex-wrap gap-3"><button onClick={() => document.getElementById("chapter-content")?.scrollIntoView({ behavior: "smooth" })} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-4 py-2.5 text-sm font-bold text-slate-950 hover:bg-cyan-200"><BookOpen size={16} /> Start reading</button><button onClick={toggleSaved} className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/5 px-4 py-2.5 text-sm font-semibold text-slate-200 hover:bg-white/10"><Star size={16} className={saved.includes(active.id) ? "fill-amber-300 text-amber-300" : ""} /> {saved.includes(active.id) ? "Saved" : "Save"}</button></div></div>
            </div>

            <article id="chapter-content" className="pb-10"><div className="mb-9 flex flex-wrap items-center gap-x-5 gap-y-2 border-y border-white/8 py-4 text-xs text-slate-500"><span className="inline-flex items-center gap-1.5"><Users size={14} /> Owner: {active.owner}</span><span className="inline-flex items-center gap-1.5"><BookOpen size={14} /> {active.readingTime} read</span><span className="inline-flex items-center gap-1.5"><CheckCircle2 size={14} className="text-emerald-400" /> Reviewed July 2026</span></div>
              {active.blocks.map((block, index) => <section key={block.heading} className="mb-10"><h2 className="mb-3 font-space text-2xl font-bold tracking-tight text-white sm:text-3xl">{block.heading}</h2><p className="text-[17px] leading-8 text-slate-300">{block.body}</p>{index === 0 && active.id === "engineering" && <Callout icon={<CheckCircle2 size={18} />} title="Best practice" text="Treat security, evaluation, documentation, and operational readiness as release gates—not work to complete after launch." tone="success" />}{index === 0 && active.id === "ai" && <Callout icon={<BrainCircuit size={18} />} title="AI recommendation" text="Benchmark against a baseline before production deployment, then continuously monitor output quality." tone="ai" />}{index === 0 && active.id === "security" && <Callout icon={<LockKeyhole size={18} />} title="Important" text="Never move client or personal information into a tool that has not been approved for the engagement." tone="warning" />}</section>)}
              <section className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 sm:p-6"><div className="mb-4 flex items-center gap-2 text-sm font-semibold text-white"><Info size={17} className="text-cyan-300" /> Related reading</div><div className="grid gap-3 sm:grid-cols-2">{sections.filter((item) => item.id !== active.id).slice(0, 4).map((item) => <button key={item.id} onClick={() => select(item.id)} className="group rounded-xl border border-white/8 bg-[#101721] p-4 text-left hover:border-cyan-300/30 hover:bg-cyan-300/[0.04]"><div className="mb-1 text-sm font-semibold text-slate-200 group-hover:text-cyan-200">{item.title}</div><div className="text-xs text-slate-500">{item.readingTime} read</div></button>)}</div></section>
            </article>
            <div className="flex items-center justify-between border-t border-white/8 pt-6"><button disabled={activeIndex <= 0} onClick={() => select(storyIds[activeIndex - 1])} className="inline-flex items-center gap-2 text-sm text-slate-400 disabled:opacity-30 hover:text-white"><ArrowLeft size={16} /> Previous</button><span className="text-xs text-slate-600">{storyMode ? "Story mode" : "Explorer mode"}</span><button disabled={activeIndex >= storyIds.length - 1} onClick={() => select(storyIds[activeIndex + 1])} className="inline-flex items-center gap-2 text-sm text-slate-400 disabled:opacity-30 hover:text-white">Next <ArrowRight size={16} /></button></div>
          </div>
        </main>

        <aside className="sticky top-[136px] hidden h-[calc(100vh-136px)] overflow-y-auto border-l border-white/8 px-5 py-10 xl:block"><p className="mb-4 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-600">On this page</p><div className="space-y-3 border-l border-white/10 pl-4">{active.blocks.map((block, i) => <a key={block.heading} href={`#chapter-content`} className={cn("block text-sm", i === 0 ? "text-cyan-300" : "text-slate-500 hover:text-slate-300")}>{block.heading}</a>)}</div><div className="mt-10 border-t border-white/8 pt-6"><p className="mb-3 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-600">Page details</p><dl className="space-y-4 text-xs"><div><dt className="text-slate-600">Owner</dt><dd className="mt-1 text-slate-300">{active.owner}</dd></div><div><dt className="text-slate-600">Tags</dt><dd className="mt-1 flex flex-wrap gap-1.5">{active.tags.map((tag) => <span key={tag} className="rounded-md bg-white/5 px-2 py-1 text-slate-400">{tag}</span>)}</dd></div><div><dt className="text-slate-600">Status</dt><dd className="mt-1 inline-flex items-center gap-1 text-emerald-400"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" /> Living document</dd></div></dl></div></aside>
      </div>
