    ...
    <div className="min-h-screen bg-[#0b0f14] pt-[72px] text-slate-100">
      ...
      <header className="sticky top-[72px] z-30 border-b border-white/8 bg-[#0b0f14]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1800px] items-center gap-3 px-4 lg:px-7">
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="grid h-9 w-9 place-items-center rounded-lg border border-border text-slate-300 hover:bg-white/5 lg:hidden" aria-label="Toggle handbook navigation"><Menu size={18} /></button>
          ...
          <div className="mx-auto hidden w-full max-w-md md:block"><label className="flex h-10 items-center gap-2 rounded-xl border border-border bg-white/[0.04] px-3 text-slate-400 focus-within:border-cyan-400/50"><Search size={16} /><input value={query} onChange={(e) => setQuery(e.target.value)} className="min-w-0 flex-1 bg-transparent text-sm text-text-primary outline-none placeholder:text-slate-500" placeholder="Search the handbook..." /><kbd className="rounded border border-border px-1.5 py-0.5 text-[10px]">⌘ K</kbd></label></div>
          ...
          <button className="hidden grid h-9 w-9 place-items-center rounded-lg border border-border text-slate-300 hover:bg-white/5 sm:grid" aria-label="More actions"><MoreHorizontal size={18} /></button>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-[1800px] grid-cols-1 lg:grid-cols-[288px_minmax(0,1fr)_250px]">
        ...
        <main className="min-w-0 px-4 py-7 sm:px-7 lg:px-12 lg:py-12">
          <div className="mx-auto max-w-[820px]">
            {query && <div className="mb-8 rounded-xl border border-border bg-white/[0.03] p-3"><p className="mb-2 text-xs text-slate-500">{results.length} matching chapters</p><div className="flex flex-wrap gap-2">{results.map((result) => <button key={result.id} onClick={() => select(result.id)} className="rounded-lg bg-white/5 px-3 py-1.5 text-xs text-cyan-200 hover:bg-cyan-400/10">{result.title}</button>)}</div></div>}
