    ...
    <div className="min-h-screen bg-[#0b0f14] pt-[72px] text-slate-100">
      ...
      <header className="sticky top-[72px] z-30 border-b border-border-default bg-[#0b0f14]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1800px] items-center gap-3 px-4 lg:px-7">
          <TrackedButton eventCategory="handbook" eventLabel="Toggle handbook navigation" onClick={() => setSidebarOpen(!sidebarOpen)} className="grid h-9 w-9 place-items-center rounded-lg border border-border text-slate-300 hover:bg-surface lg:hidden" aria-label="Toggle handbook navigation"><Menu size={18} /></TrackedButton>
          ...
          <TrackedButton eventCategory="handbook" eventLabel={storyMode ? "Explorer" : "Story mode"} onClick={() => setStoryMode(!storyMode)} className="ml-auto rounded-lg border border-border px-3 py-2 text-xs font-semibold text-slate-300 hover:border-cyan-400/40 hover:text-cyan-300">{storyMode ? "Explorer" : "Story mode"}</TrackedButton>
          <TrackedButton eventCategory="handbook" eventLabel="More actions" className="hidden grid h-9 w-9 place-items-center rounded-lg border border-border text-slate-300 hover:bg-surface sm:grid" aria-label="More actions"><MoreHorizontal size={18} /></TrackedButton>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-[1800px] grid-cols-1 lg:grid-cols-[288px_minmax(0,1fr)_250px]">
        <aside className={cn("fixed inset-y-0 left-0 z-50 w-[288px] overflow-y-auto border-r border-border-default bg-[#0d1219] px-4 pb-8 pt-24 transition-transform lg:sticky lg:top-[136px] lg:z-10 lg:h-[calc(100vh-136px)] lg:translate-x-0 lg:pt-6", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
          <div className="mb-6 flex items-center justify-between lg:hidden"><span className="font-space font-semibold">Handbook</span><TrackedButton eventCategory="handbook" eventLabel="Close handbook navigation" onClick={() => setSidebarOpen(false)}><X size={18} /></TrackedButton></div>
          ...
          <nav className="space-y-1" aria-label="Handbook chapters">
            {sections.map((section) => { const Icon = section.icon; const selected = activeId === section.id; return <TrackedButton key={section.id} eventCategory="handbook" eventLabel={section.title} onClick={() => select(section.id)} className={cn("group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition", selected ? "bg-surface-elevated text-text-primary shadow-sm" : "text-slate-400 hover:bg-surface-elevated hover:text-slate-200")}><Icon size={16} className={selected ? "text-cyan-300" : "text-slate-500 group-hover:text-cyan-300"} /><span className="flex-1">{section.title}</span>{saved.includes(section.id) && <Star size={13} className="fill-amber-300 text-amber-300" />}{selected && <ChevronRight size={15} className="text-slate-500" />}</TrackedButton>; })}
          </nav>
          <div className="mt-7 border-t border-border-default pt-5"><p className="mb-3 px-2 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-600">Quick access</p><TrackedButton eventCategory="handbook" eventLabel="Favorites" className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-sm text-slate-400 hover:text-text-primary"><Star size={15} /> Favorites <span className="ml-auto text-xs text-slate-600">{saved.length}</span></TrackedButton><TrackedButton eventCategory="handbook" eventLabel="Playbooks" className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-sm text-slate-400 hover:text-text-primary"><ClipboardCheck size={15} /> Playbooks</TrackedButton></div>
        </aside>
        {sidebarOpen && <TrackedButton eventCategory="handbook" eventLabel="Close navigation" onClick={() => setSidebarOpen(false)} className="fixed inset-0 z-40 bg-surface-elevated lg:hidden" aria-label="Close navigation" />}

        <main className="min-w-0 px-4 py-7 sm:px-7 lg:px-12 lg:py-12">
          <div className="mx-auto max-w-[820px]">
            {query && <div className="mb-8 rounded-xl border border-border bg-surface-elevated p-3"><p className="mb-2 text-xs text-slate-500">{results.length} matching chapters</p><div className="flex flex-wrap gap-2">{results.map((result) => <TrackedButton key={result.id} eventCategory="handbook" eventLabel={result.title} onClick={() => select(result.id)} className="rounded-lg bg-surface px-3 py-1.5 text-xs text-cyan-200 hover:bg-cyan-400/10">{result.title}</TrackedButton>)}</div></div>}
            <div className="relative mb-10 overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-[#111c2a] via-[#111522] to-[#19122b] p-7 shadow-2xl shadow-cyan-950/20 sm:p-10">
              ...
              <div className="relative"><div className="mb-5 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-cyan-300"><Sparkles size={14} /> {active.eyebrow}</div><h1 className="max-w-xl font-space text-4xl font-bold tracking-tight text-text-primary sm:text-5xl">{active.title}</h1><p className="mt-5 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">{active.description}</p><div className="mt-7 flex flex-wrap gap-3"><TrackedButton eventCategory="handbook" eventLabel="Start reading" onClick={() => document.getElementById("chapter-content")?.scrollIntoView({ behavior: "smooth" })} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-4 py-2.5 text-sm font-bold text-slate-950 hover:bg-cyan-200"><BookOpen size={16} /> Start reading</TrackedButton><TrackedButton eventCategory="handbook" eventLabel={saved.includes(active.id) ? "Saved" : "Save"} onClick={toggleSaved} className="inline-flex items-center gap-2 rounded-xl border border-border-default bg-surface px-4 py-2.5 text-sm font-semibold text-slate-200 hover:bg-surface-elevated"><Star size={16} className={saved.includes(active.id) ? "fill-amber-300 text-amber-300" : ""} /> {saved.includes(active.id) ? "Saved" : "Save"}</TrackedButton></div></div>
            </div>
            ...
            <article id="chapter-content" className="pb-10"><div className="mb-9 flex flex-wrap items-center gap-x-5 gap-y-2 border-y border-border-default py-4 text-xs text-slate-500"><span className="inline-flex items-center gap-1.5"><Users size={14} /> Owner: {active.owner}</span><span className="inline-flex items-center gap-1.5"><BookOpen size={14} /> {active.readingTime} read</span><span className="inline-flex items-center gap-1.5"><CheckCircle2 size={14} className="text-emerald-400" /> Reviewed July 2026</span></div>
              ...
              <section className="rounded-2xl border border-border bg-surface-elevated p-5 sm:p-6"><div className="mb-4 flex items-center gap-2 text-sm font-semibold text-text-primary"><Info size={17} className="text-cyan-300" /> Related reading</div><div className="grid gap-3 sm:grid-cols-2">{sections.filter((item) => item.id !== active.id).slice(0, 4).map((item) => <TrackedButton key={item.id} eventCategory="handbook" eventLabel={item.title} onClick={() => select(item.id)} className="group rounded-xl border border-border-default bg-[#101721] p-4 text-left hover:border-cyan-300/30 hover:bg-cyan-300/[0.04]"><div className="mb-1 text-sm font-semibold text-slate-200 group-hover:text-cyan-200">{item.title}</div><div className="text-xs text-slate-500">{item.readingTime} read</div></TrackedButton>)}</div></section>
            </article>
            <div className="flex items-center justify-between border-t border-border-default pt-6"><TrackedButton eventCategory="handbook" eventLabel="Previous chapter" disabled={activeIndex <= 0} onClick={() => select(storyIds[activeIndex - 1])} className="inline-flex items-center gap-2 text-sm text-slate-400 disabled:opacity-30 hover:text-text-primary"><ArrowLeft size={16} /> Previous</TrackedButton><span className="text-xs text-slate-600">{storyMode ? "Story mode" : "Explorer mode"}</span><TrackedButton eventCategory="handbook" eventLabel="Next chapter" disabled={activeIndex >= storyIds.length - 1} onClick={() => select(storyIds[activeIndex + 1])} className="inline-flex items-center gap-2 text-sm text-slate-400 disabled:opacity-30 hover:text-text-primary">Next <ArrowRight size={16} /></TrackedButton></div>
          </div>
        </main>

        <aside className="sticky top-[136px] hidden h-[calc(100vh-136px)] overflow-y-auto border-l border-border-default px-5 py-10 xl:block"><p className="mb-4 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-600">On this page</p><div className="space-y-3 border-l border-border pl-4">{active.blocks.map((block, i) => <TrackedLink key={block.heading} href={`#chapter-content`} analyticsEvent="handbook_toc_click" analyticsCategory="handbook" analyticsLabel={block.heading} className={cn("block text-sm", i === 0 ? "text-cyan-300" : "text-slate-500 hover:text-slate-300")}>{block.heading}</TrackedLink>)}</div><div className="mt-10 border-t border-border-default pt-6"><p className="mb-3 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-600">Page details</p><dl className="space-y-4 text-xs"><div><dt className="text-slate-600">Owner</dt><dd className="mt-1 text-slate-300">{active.owner}</dd></div><div><dt className="text-slate-600">Tags</dt><dd className="mt-1 flex flex-wrap gap-1.5">{active.tags.map((tag) => <span key={tag} className="rounded-md bg-surface px-2 py-1 text-slate-400">{tag}</span>)}</dd></div><div><dt className="text-slate-600">Status</dt><dd className="mt-1 inline-flex items-center gap-1 text-emerald-400"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" /> Living document</dd></div></dl></div></aside>
      </div>
      <TrackedButton eventCategory="handbook" eventLabel="Ask Handbook AI" className="fixed bottom-5 right-5 z-30 inline-flex items-center gap-2 rounded-full border border-cyan-300/30 bg-[#141c28] px-4 py-3 text-sm font-semibold text-cyan-200 shadow-xl shadow-black/30 hover:bg-[#192536]"><Sparkles size={16} /> Ask Handbook AI</TrackedButton>
    </div>
  );
}
