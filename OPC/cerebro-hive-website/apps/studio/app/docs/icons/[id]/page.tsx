import { Icon } from "@/components/cerebro/icons/Icon";
import { iconRegistry } from "@/components/cerebro/icons/IconRegistry";
import { notFound } from "next/navigation";

// For static generation
export function generateStaticParams() {
  return Object.values(iconRegistry).map(({ id }) => ({
    id,
  }));
}

export default async function IconDocumentationPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id: iconId } = await params;
  const registryEntry = Object.entries(iconRegistry).find(([, metadata]) => metadata.id === iconId);

  if (!registryEntry) {
    notFound();
  }

  const [registeredName, meta] = registryEntry;
  ...
  const componentName = meta.displayName || registeredName;
    ...
    <div className="max-w-4xl mx-auto py-12 px-6">
      <div className="mb-8 border-b border-border pb-6">
        ...
        <div className="flex gap-2 mb-4">
          ...
          <span className="px-3 py-1 bg-surface-elevated text-text-secondary text-sm rounded-full capitalize">
            {meta.category}
          </span>
        </div>
        <p className="text-text-muted text-lg">
          Category: {meta.category} {meta.subcategory && `/ ${meta.subcategory}`}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        <div
          ...
          tabIndex={0}
        >
          <Icon name={iconId} size={96} decorative={false} label={`${componentName} preview`} />
        </div>
        ...
        <div className="space-y-6">
          ...
          <section>
            ...
            <ul className="space-y-2 text-text-secondary">
              <li>
                <strong className="text-text-primary">Introduced:</strong> {meta.introduced}
              </li>
              {meta.keywords && meta.keywords.length > 0 && (
                <li>
                  <strong className="text-text-primary">Keywords:</strong>{" "}
                  {meta.keywords.join(", ")}
                </li>
              )}
              {meta.aliases && meta.aliases.length > 0 && (
                <li>
                  <strong className="text-text-primary">Aliases:</strong> {meta.aliases.join(", ")}
                </li>
              )}
            </ul>
          </section>
      ...
      <section>
        ...
        <p className="text-text-secondary mb-4">
          This icon inherits all motion states from the global animation taxonomy. Use the
          `animation` prop to apply states like `pulse`, `orbit`, or `scan`.
        </p>
        <div className="bg-surface-elevated p-4 rounded-lg overflow-x-auto">
          <pre className="text-sm">
