...
const ROLE_COLOR: Record<string, string> = {
  ...
  research: "border-teal-400/50 text-teal-300",
  "cloud-architect": "border-sky-400/50 text-sky-300",
  "ai-architect": "border-fuchsia-400/50 text-fuchsia-300",
  "solution-architect": "border-orange-400/50 text-orange-300",
      ...
      <button onClick={() => void toggle()} className="flex w-full items-center justify-between gap-3 text-left">
        <div>
          ...
          <div className="mt-1 text-xs text-text-secondary">
            {directive.priority} priority · {directive.status}
            {directive.verdict && <> · <span className={directive.verdict === "approved" ? "text-primary-accent" : "text-amber-400"}>{directive.verdict === "approved" ? "approved" : "max revisions reached"}</span></>}
            {directive.revisions > 0 && <> · {directive.revisions} revision{directive.revisions === 1 ? "" : "s"}</>}
            {" "}· submitted {new Date(directive.createdAt).toLocaleString()}
          </div>
        </div>
          ...
          {tasks?.map(t => (
            <div key={t.id} className={`rounded-lg border ${ROLE_COLOR[t.role] ?? "border-border"} bg-surface-elevated/40 p-3`}>
              <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
                ...
                <span className="uppercase tracking-wider">{t.recipient}{t.attempt > 1 ? ` · revision ${t.attempt}` : ""}</span>
              </div>
              <div className="mt-1 text-xs text-text-secondary">
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 pb-24 pt-8">
      ...
      <p className="mt-3 max-w-2xl text-text-secondary">
        Nine named domain-expert agents — CEO, Strategy, Enterprise Architect, Project Planner, Research, Cloud
        Architect, AI Architect, Solution Architect, and Reviewer — coordinate every submitted directive through a
        sequential chain of real, model-grounded reasoning (each stage sees the actual output of every prior stage,
        not a canned response). The Reviewer can reject and send the directive back to an earlier stage with
        specific feedback, bounded to a couple of revision rounds. All nine roles are registered as real Agent Mesh
        agents, discoverable and votable alongside every other agent on the platform. These agents plan and
        coordinate; they don&apos;t write code. Engineering, DevOps, Security, and Business execution swarms are a
        later phase once this coordination layer is validated.
      </p>
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3">
          {agents.map(a => (
      ...
      <section className="mt-8 rounded-xl border border-border bg-surface/40 p-5">
        ...
        <p className="mt-1 text-xs text-text-secondary">The directive runs through all nine agents in order — CEO → Strategy → Enterprise Architect → Planner → Research → Cloud Architect → AI Architect → Solution Architect → Reviewer — with real reasoning at each stage and possible revision rounds.</p>
