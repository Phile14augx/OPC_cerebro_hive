...
export class EffectRecorder {
  ...
  static record(executionId: string, effectId: string, responsePayload: unknown) {
...
export class VirtualEffectLayer {
  ...
  static async intercept(executionId: string, effectId: string, requestPayload: unknown): Promise<{ status: string }> {
    void executionId;
    void requestPayload;
