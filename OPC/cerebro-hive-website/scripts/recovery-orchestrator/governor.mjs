...
function recommendedExampleCommand(input) {
  const recommended = input?.evidenceProgress?.recommendedCommands;
  if (Array.isArray(recommended) && recommended.length > 0) {
    return recommended[0];
  }
  return { exe: "git", args: ["rev-parse", "HEAD"] };
}
...
function protocolShape(input) {
  ...
  return {
    ...
    unknowns: ["missing fact or outstanding objective"],
    writeAuthorized: false,
    nextAction: {
      actionId,
      mode: "READ_ONLY",
      repository: input?.state?.repository ?? "absolute path supplied in state",
      commands: [recommendedExampleCommand(input)],
      allowedPaths: [],
      forbiddenPaths: [],
      acceptanceCriteria: ["new raw evidence captured for the next outstanding objective"],
      stopConditions: ["stop on command failure or unexpected repository mutation"],
    },
  };
}
...
export class QwenGovernorAdapter {
  ...
  async decide(input) {
    ...
    const progressInstruction = input?.evidenceProgress
      ? `Evidence progress is authoritative for planning. Outstanding objectives: ${input.evidenceProgress.outstanding.join(", ") || "none"}. Next objective: ${input.evidenceProgress.nextObjective?.id ?? "none"}. Do not repeat any exact successful command fingerprint. Prefer the supplied recommendedCommands when they exist.`
      : "";
    const feedbackInstruction = input?.planningFeedback
      ? `The orchestrator rejected the previous plan: ${input.planningFeedback.rejectedPlan}. Replan now and obey the supplied nextObjective/recommendedCommands.`
      : "";
    const baseMessages = [
