...
25. Evidence collection must advance the recovery proof. Do not repeat an exact command that already completed successfully unless new evidence proves the inspected target changed and a re-check is necessary.
26. When `evidenceProgress` is supplied, treat its OUTSTANDING objectives as unresolved. Do not claim "all evidence is valid/complete/consistent", "no conflicts", or "no unknowns" while any required objective remains outstanding. Prefer the next objective and recommended deterministic commands.
27. An immutable `git show <ref>:<path>` result proving that a path does not exist at that ref is evidence about that ref, not a terminal repository failure. Record the absence, do not repeat the same missing-path probe, and inspect an authorized immutable candidate ref when the recovery objective is to reconcile baseline and candidate sources.
