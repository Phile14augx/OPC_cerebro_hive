      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-6 border-b border-border pb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-space font-bold tracking-widest uppercase text-text-primary">CerebroHive</span>
          </div>
        </div>

        <div className="flex gap-5 items-center mb-6">
          <div className="w-20 h-20 rounded-xl overflow-hidden bg-surface relative shadow-inner border border-border">
            {data.avatar && <Image src={withBasePath(data.avatar)} alt={data.title} fill className="object-cover" />}
          </div>
          <div>
            <h3 className="text-xl font-space font-bold text-text-primary leading-tight mb-1">{data.title}</h3>
        ...
        {data.metrics && (
          <div className="flex justify-between border-t border-border pt-4 mt-2">
            <div className="text-center">
              <span className="block text-lg font-space font-bold text-text-primary">{data.metrics.employees}</span>
              <span className="block text-[10px] font-inter text-text-muted uppercase">Employees</span>
            </div>
            <div className="text-center">
              <span className="block text-lg font-space font-bold text-text-primary">{data.metrics.teams}</span>
              <span className="block text-[10px] font-inter text-text-muted uppercase">Teams</span>
            </div>
            <div className="text-center">
              <span className="block text-lg font-space font-bold text-text-primary">{data.metrics.countries}</span>
