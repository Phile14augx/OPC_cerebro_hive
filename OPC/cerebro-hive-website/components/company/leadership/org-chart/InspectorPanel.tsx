...
export const InspectorPanel = () => {
  ...
  return (
    <Panel position="top-right" className="!m-4 flex flex-col gap-4 pointer-events-none z-50">
      ...
      <div className="w-[340px] bg-background/80 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-2xl flex flex-col overflow-hidden pointer-events-auto transition-all">
        {node ? (
          /* ACTIVE STATE: Node Selected */
          <div className="flex flex-col h-full max-h-[500px]">
            {/* Header */}
            <div className="flex items-start justify-between p-5 border-b border-white/5 bg-white/[0.02]">
              <div>
                ...
                <h2 className="text-lg font-space font-bold text-white mb-0.5 leading-tight">{node.title}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-6 custom-scrollbar">
              ...
              {node.avatar && (
                <div className="flex items-center gap-3 bg-surface p-3 rounded-xl border border-white/5">
                  <div className="w-10 h-10 rounded-full overflow-hidden relative border border-white/10 shadow-inner shrink-0">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(node.metrics).map(([key, value]) => (
                      <div key={key} className="bg-surface border border-white/5 p-2.5 rounded-lg flex flex-col gap-0.5">
                        <span className="text-[9px] font-mono text-text-muted capitalize">{key}</span>
                        <span className="text-base font-space font-bold text-white">{value}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="p-3 border-t border-white/5">
              <button className="w-full py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-[11px] font-space font-bold text-white transition-colors flex items-center justify-center gap-2">
          ...
          <div className="flex flex-col">
            <div className="p-5 border-b border-white/5 bg-white/[0.02]">
              <div className="inline-block px-2 py-1 rounded text-[9px] font-mono uppercase tracking-widest mb-2 border border-white/10 text-text-muted bg-white/5">
                Global Network
              </div>
              <h2 className="text-lg font-space font-bold text-white mb-0.5">Company Overview</h2>
              <p className="text-[11px] text-text-secondary font-inter">CerebroHive Organization Intelligence</p>
            </div>
            
            <div className="p-5 flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-surface border border-white/5 p-2.5 rounded-lg">
                  <span className="text-[9px] font-mono text-text-muted">Total People</span>
                  <span className="block text-base font-space font-bold text-white">68</span>
                </div>
                <div className="bg-surface border border-white/5 p-2.5 rounded-lg">
                  <span className="text-[9px] font-mono text-text-muted">Divisions</span>
                  <span className="block text-base font-space font-bold text-white">4</span>
                </div>
              </div>
      ...
      {/* Tools & MiniMap Card */}
      <div className="w-[340px] bg-background/80 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-xl flex flex-col overflow-hidden pointer-events-auto">
        <div className="p-2 border-b border-white/5 flex justify-between gap-1 bg-white/[0.02]">
