    ...
    <section className="py-24 border-b border-white/5 bg-[#0A0D14]">
      <div className="container-wide">
        ...
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-space font-bold text-white mb-6">The Breadth of our Ecosystem</h2>
          ...
          {categories.map((cat) => (
            <button
              ...
              className={cn(
                ...
                activeCategory === cat.id ? "bg-white/10 border-white/30 shadow-[0_0_20px_rgba(255,255,255,0.05)] scale-105" : "bg-surface border-white/5 hover:border-white/20"
              )}
            >
              <cat.icon size={24} className={activeCategory === cat.id ? "text-white" : "text-text-muted"} />
              <div className="font-space font-bold text-sm text-white">{cat.label}</div>
            </button>
          ))}
        </div>

        <div className="bg-surface border border-white/10 rounded-2xl p-8 md:p-16 min-h-[300px] flex items-center justify-center text-center relative overflow-hidden">
          <AnimatePresence mode="wait">
            {categories.map(cat => cat.id === activeCategory && (
              ...
              >
                <div className="text-6xl md:text-8xl font-space font-bold text-white mb-4 tracking-tighter">
                  {cat.stat}
                </div>
                <div className="text-xl md:text-2xl text-text-secondary font-inter">
                  {cat.subtitle}
                </div>
                <button className="mt-8 px-6 py-3 border border-white/20 rounded-lg text-xs font-bold uppercase tracking-widest text-white hover:bg-white/5 transition-colors">
                  Explore {cat.label}
                </button>
              </motion.div>
            ))}
