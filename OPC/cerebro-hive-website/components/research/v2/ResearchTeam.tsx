    ...
    <section className="py-24 border-b border-white/5 bg-[#0A0D14]">
      <div className="container-wide">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div>
            <h2 className="text-3xl md:text-5xl font-space font-bold text-white mb-4">Research Authors</h2>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {researchers.map((person, i) => (
            <div key={i} className="bg-surface border border-white/10 rounded-2xl p-6 hover:border-white/20 transition-colors">
              
              <div className="flex items-center gap-4 mb-6">
                <div className="w-16 h-16 rounded-full bg-black/50 border border-white/10 overflow-hidden flex items-center justify-center shrink-0">
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {person.interests.map((interest, j) => (
                    <span key={j} className="px-2 py-1 rounded bg-white/5 border border-white/10 text-xs text-text-secondary">
                      {interest}
                    </span>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-3 gap-2 py-4 border-t border-white/5 mb-4 text-center">
                <div>
                  <div className="text-white font-bold flex items-center justify-center gap-1"><FileText size={12} className="text-text-muted"/> {person.stats.papers}</div>
                  <div className="text-[9px] uppercase tracking-widest text-text-muted mt-1">Papers</div>
                </div>
                <div>
                  <div className="text-white font-bold flex items-center justify-center gap-1"><FlaskConical size={12} className="text-[#00E5FF]"/> {person.stats.experiments}</div>
                  <div className="text-[9px] uppercase tracking-widest text-text-muted mt-1">Exps</div>
                </div>
                <div>
                  <div className="text-white font-bold">{person.stats.talks}</div>
