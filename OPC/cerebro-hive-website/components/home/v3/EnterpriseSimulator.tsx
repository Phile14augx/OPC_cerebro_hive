    ...
    <section className="py-24 border-b border-white/5 bg-[#05070A] relative overflow-hidden">
      ...
      <div className="container-wide">
        ...
        <div className="text-center mb-16 relative z-10">
          ...
          <h2 className="text-3xl md:text-5xl font-space font-bold text-white mb-6">Enterprise Transformation Simulator</h2>
        ...
        <div className="max-w-5xl mx-auto relative z-10">
              ...
              <div className="flex items-center justify-between mb-12 relative max-w-lg mx-auto">
                ...
                {[1, 2, 3].map(num => (
                  <div key={num} className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold relative z-10 transition-colors ${
                    ...
                    step > num ? "bg-[#00F57A] text-black border-4 border-[#0A0D14]" :
                    "bg-black text-text-muted border border-white/20"
                  }`}>
              ...
              <AnimatePresence mode="wait">
                {step === 1 && (
                  <motion.div key="1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                    <div className="text-center mb-8">
                      <Building2 size={32} className="text-[#00E5FF] mx-auto mb-4" />
                      <h3 className="text-2xl font-space font-bold text-white">Select Industry</h3>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
                      {["Finance", "Healthcare", "Manufacturing", "Retail"].map(ind => (
                        <button key={ind} onClick={() => handleSelect('industry', ind)} className="p-4 rounded-xl border border-white/10 bg-white/5 font-bold text-white hover:border-[#00E5FF] hover:bg-[#00E5FF]/10 transition-colors">
                          {ind}
                        </button>
                      ))}
                    </div>
                ...
                {step === 2 && (
                  <motion.div key="2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                    <div className="text-center mb-8">
                      <Server size={32} className="text-[#00E5FF] mx-auto mb-4" />
                      <h3 className="text-2xl font-space font-bold text-white">Primary ERP / System</h3>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
                      {["SAP", "Oracle", "Workday", "Custom/Legacy"].map(erp => (
                        <button key={erp} onClick={() => handleSelect('erp', erp)} className="p-4 rounded-xl border border-white/10 bg-white/5 font-bold text-white hover:border-[#00E5FF] hover:bg-[#00E5FF]/10 transition-colors">
                          {erp}
                        </button>
                      ))}
                    </div>
                ...
                {step === 3 && (
                  <motion.div key="3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                    <div className="text-center mb-8">
                      <Target size={32} className="text-[#00E5FF] mx-auto mb-4" />
                      <h3 className="text-2xl font-space font-bold text-white">Current AI Maturity</h3>
                    </div>
                    <div className="grid md:grid-cols-3 gap-4 max-w-3xl mx-auto">
                      {["Exploration (Pilots)", "Isolated (Copilots)", "Integrated (APIs)"].map(mat => (
                        <button key={mat} onClick={() => handleSelect('maturity', mat)} className="p-4 rounded-xl border border-white/10 bg-white/5 font-bold text-white hover:border-[#00E5FF] hover:bg-[#00E5FF]/10 transition-colors">
                          {mat}
                        </button>
                      ))}
                    </div>
          ...
          {generating && (
            <div className="bg-surface border border-white/10 rounded-2xl p-16 text-center flex flex-col items-center shadow-2xl">
              <div className="w-16 h-16 rounded-full border-4 border-white/10 border-t-[#00E5FF] animate-spin mb-8" />
              <h3 className="text-2xl font-space font-bold text-white mb-2">Architecting Blueprint...</h3>
          ...
          <AnimatePresence>
            {generated && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-surface border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
                ...
                <div className="p-8 md:p-12">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                    <div>
                      <h3 className="text-3xl font-space font-bold text-white mb-2">Executive AI Blueprint</h3>
                  ...
                  <div className="grid md:grid-cols-3 gap-8 mb-12">
                    <div className="bg-black/50 border border-white/5 rounded-xl p-6">
                      ...
                      <div className="text-4xl font-space font-bold text-white mb-2">42<span className="text-lg text-text-muted">/100</span></div>
                      <p className="text-xs text-text-secondary">Your current setup is highly dependent on manual prompt engineering.</p>
                    </div>
                    <div className="bg-black/50 border border-white/5 rounded-xl p-6">
                      <div className="text-[10px] uppercase tracking-widest text-[#00E5FF] font-bold mb-2">Top Opportunity</div>
                      <div className="text-xl font-space font-bold text-white mb-2">Workflow Automation</div>
                      <p className="text-xs text-text-secondary">Deploying AgentOS alongside {selections.erp} can automate 60% of L1 tasks.</p>
                    </div>
                    <div className="bg-black/50 border border-white/5 rounded-xl p-6">
                      <div className="text-[10px] uppercase tracking-widest text-[#00F57A] font-bold mb-2">Estimated ROI</div>
                      <div className="text-4xl font-space font-bold text-white mb-2">2.8x</div>
                      <p className="text-xs text-text-secondary">Expected return within 12 months based on headcount reallocation.</p>
                    </div>
                  </div>

                  <h4 className="font-space font-bold text-white text-xl mb-6">Recommended Architecture Path</h4>
                  <div className="grid md:grid-cols-3 gap-4 mb-8">
                    <div className="p-5 bg-white/5 border border-white/10 rounded-xl relative">
                      <Layers size={20} className="text-text-muted mb-4" />
                      <h5 className="font-bold text-white mb-2">1. Knowledge Layer</h5>
                      <p className="text-xs text-text-secondary">Vectorize {selections.erp} data using Cerebro Knowledge Hub.</p>
                    </div>
                    <div className="p-5 bg-white/5 border border-white/10 rounded-xl relative border-[#00E5FF]/30">
                      <Bot size={20} className="text-[#00E5FF] mb-4" />
                      <h5 className="font-bold text-white mb-2">2. Agent Network</h5>
                    ...
                    <div className="p-5 bg-white/5 border border-white/10 rounded-xl relative">
                      ...
                      <h5 className="font-bold text-white mb-2">3. Decision Engine</h5>
