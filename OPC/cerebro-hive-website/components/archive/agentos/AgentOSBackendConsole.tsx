 ...
 * this one requires the backend to be running locally:
 *
 *   cd agentos && pip install -r requirements.txt --break-system-packages
 *   python scripts/seed.py && uvicorn app.main:app --port 8088
 */
export const AgentOSBackendConsole = () => {
    ...
    <div className="flex flex-col gap-8">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-surface border border-border shadow-sm">
          <span className={cn("w-2 h-2 rounded-full", connection === "online" ? "bg-primary-accent animate-pulse" : connection === "checking" ? "bg-yellow-500" : "bg-red-500")} />
          <span className="text-[10px] font-bold tracking-widest uppercase text-text-secondary">
            {connection === "online" ? "Connected to the Python AgentOS backend" : connection === "checking" ? "Connecting…" : "Backend offline"}
          </span>
        </div>
        ...
        <div className="bg-surface border border-red-500/30 rounded-2xl p-6 flex gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-text-secondary mb-4">
              ...
              <code className="bg-background border border-border rounded px-1">uvicorn app.main:app --port 8088</code>.
            </p>
