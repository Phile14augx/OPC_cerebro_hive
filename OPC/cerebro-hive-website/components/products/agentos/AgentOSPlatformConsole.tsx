...
export const AgentOSPlatformConsole = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!health.ok) return;
        const key = await api(apiBase, null, "/auth/api-keys", { method: "POST", body: JSON.stringify({ owner: "platform-console" }) });
...
function SimulatorPanel({ apiBase, apiKey }: { apiBase: string; apiKey: string | null }) {
  ...
  return (
    <Card>
      <h3 className="font-space font-bold text-text-primary text-sm mb-4">Monte Carlo digital twin — "what if we staffed N agents?"</h3>
