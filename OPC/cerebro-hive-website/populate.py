...
workspaces = [
    "admin", "agents", "capabilities", "dashboard", "evaluation", "events", 
    "governance", "knowledge", "models", "prompts", "runtime", "settings", 
    "simulation", "traces", "workflows"
]

base_path = r"d:\{MY_PROJECTS}\{OPC_cerebro_hive}\OPC\cerebro-hive-website\app\(platform)\studio"
...
export default function {component_name}Page() {{
  return (
    <StudioShell activeWorkspace="{workspace_name}">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <WorkspaceCard title="{workspace_name} Overview" className="lg:col-span-2 min-h-[400px]">
          <div className="flex items-center justify-center h-full text-neutral-500 border-2 border-dashed border-neutral-800 rounded-xl">
            [ Premium {workspace_name} UI Widget ]
          </div>
        </WorkspaceCard>
...
for w in workspaces:
    component_name = w.capitalize()
    workspace_name = w.capitalize()
    content = template.format(component_name=component_name, workspace_name=workspace_name)
    
    file_path = os.path.join(base_path, w, "page.tsx")
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(content)

print("Successfully populated 15 workspace pages.")
