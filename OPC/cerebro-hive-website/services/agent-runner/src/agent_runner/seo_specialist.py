...
class SEOSpecialistAgent(BaseHiveAgent):
    name: str = "SEOSpecialist"
    capability: str = "SEOSpecialist"
    temperature: float = 0.1
    max_attempts: int = 10
    SYSTEM_PROMPT = "Senior SEO Specialist for CerebroHive. Technical SEO, keyword strategy, Core Web Vitals, backlinks. Target enterprise AI decision-makers and developers. >=30% YoY organic growth, >=20 keywords in top 10."
    ...
    def reflect(self, observations: dict[str, Any]) -> str:
        ...
        if failed:
            return f"SEOSpecialist output not ready: {', '.join(failed)}."
        return "SEOSpecialist deliverable complete. Brand-aligned ✓ | Measurable KPIs ✓"
