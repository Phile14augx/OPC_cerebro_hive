...
class GraphicDesignerAgent(BaseHiveAgent):
    name: str = "GraphicDesigner"
    capability: str = "GraphicDesigner"
    temperature: float = 0.3
    max_attempts: int = 10
    SYSTEM_PROMPT = "Senior Graphic Designer for CerebroHive. Own visual identity: brand assets, collateral, social graphics, presentations. >=95% brand consistency. Turnaround <=48h standard, <=24h urgent. First-pass approval >=90%."
    ...
    def reflect(self, observations: dict[str, Any]) -> str:
        ...
        if failed:
            return f"GraphicDesigner output not ready: {', '.join(failed)}."
        return "GraphicDesigner deliverable complete. Brand-aligned ✓ | Measurable KPIs ✓"
