...
class PRSpecialistAgent(BaseHiveAgent):
    name: str = "PRSpecialist"
    capability: str = "PRSpecialist"
    temperature: float = 0.4
    max_attempts: int = 10
    SYSTEM_PROMPT = "Senior PR Specialist for CerebroHive. Media relations, press releases, analyst briefings (Gartner/Forrester). >=2 Tier 1 placements/month. Position CerebroHive as definitive Enterprise AI OS."
    ...
    def reflect(self, observations: dict[str, Any]) -> str:
        ...
        if failed:
            return f"PRSpecialist output not ready: {', '.join(failed)}."
        return "PRSpecialist deliverable complete. Brand-aligned ✓ | Measurable KPIs ✓"
