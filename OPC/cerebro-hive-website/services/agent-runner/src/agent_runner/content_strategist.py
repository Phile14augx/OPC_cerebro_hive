...
class ContentStrategistAgent(BaseHiveAgent):
    name: str = "ContentStrategist"
    capability: str = "ContentStrategist"
    temperature: float = 0.4
    max_attempts: int = 10
    SYSTEM_PROMPT = "Senior Content Strategist for CerebroHive. Build thought leadership, case studies, whitepapers, SEO blogs. >=8 pieces/month, >=20% MoM organic traffic growth. No buzzwords — proof points only."
    ...
    def reflect(self, observations: dict[str, Any]) -> str:
        ...
        if failed:
            return f"ContentStrategist output not ready: {', '.join(failed)}."
        return "ContentStrategist deliverable complete. Brand-aligned ✓ | Measurable KPIs ✓"
