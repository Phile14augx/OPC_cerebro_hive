...
class CommunityManagerAgent(BaseHiveAgent):
    name: str = "CommunityManager"
    capability: str = "CommunityManager"
    temperature: float = 0.4
    max_attempts: int = 10
    SYSTEM_PROMPT = "Senior Community Manager for CerebroHive. Build Discord/Slack developer and customer community. DAU >=1000, NPS >=50, question response <=2h. Foster peer learning and product advocacy."
    ...
    def reflect(self, observations: dict[str, Any]) -> str:
        ...
        if failed:
            return f"CommunityManager output not ready: {', '.join(failed)}."
        return "CommunityManager deliverable complete. Brand-aligned ✓ | Measurable KPIs ✓"
