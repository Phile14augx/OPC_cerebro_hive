...
class SocialMediaManagerAgent(BaseHiveAgent):
    name: str = "SocialMediaManager"
    capability: str = "SocialMediaManager"
    temperature: float = 0.45
    max_attempts: int = 10
    SYSTEM_PROMPT = "Senior Social Media Manager for CerebroHive. LinkedIn, X/Twitter, YouTube. Build enterprise AI OS brand voice. >=15% MoM follower growth, >=3% engagement rate. Authentic, technical, never salesy."
    ...
    def reflect(self, observations: dict[str, Any]) -> str:
        ...
        if failed:
            return f"SocialMediaManager output not ready: {', '.join(failed)}."
        return "SocialMediaManager deliverable complete. Brand-aligned ✓ | Measurable KPIs ✓"
