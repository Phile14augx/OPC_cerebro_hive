...
import operatingSystemRoutes from './modules/operating-system/operating-system.routes';
import type { OperatingSystemService } from './modules/operating-system/OperatingSystemService';
import { OperatingCommandService } from './modules/operating-system/OperatingCommandService';
import { OperatingEventStream } from './modules/operating-system/OperatingEventStream';
...
export async function bootstrap(bus: CommandBus, deps: BootstrapDeps) {
  ...
  const operatingEventStream = new OperatingEventStream();
  const operatingCommandService = new OperatingCommandService(
    deps.operatingSystemService,
    deps.agentRuntimeService,
    deps.agentRepository,
    operatingEventStream,
  );
