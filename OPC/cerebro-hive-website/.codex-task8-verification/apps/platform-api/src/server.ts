...
import { AgentRepository, AgentConversationRepository, IdempotencyRepository, OutboxRepository, AuditRepository, WorkspaceRepository, PrismaUnitOfWork, OperatingSystemRepository } from '@cerebro/db';
...
import { OperatingSystemService } from './modules/operating-system/OperatingSystemService';
...
async function main() {
  ...
  const operatingSystemRepo = new OperatingSystemRepository(prisma);
  const operatingSystemService = new OperatingSystemService(operatingSystemRepo);
