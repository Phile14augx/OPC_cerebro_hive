import { IdentityRepository, Principal, IdentityContext, HumanPrincipal, FederatedIdentity } from '@cerebro/identity-core';
  ...
  async authenticate(token: string, mappingProfile: MappingProfile): Promise<IdentityContext> {
    ...
    if (!principal) {
      ...
      const newPrincipalData: Omit<HumanPrincipal, 'id'> = {
        type: 'Human',
        status: 'Active',
        displayName: validationResult.claims.name || validationResult.claims.email || 'Unknown User',
