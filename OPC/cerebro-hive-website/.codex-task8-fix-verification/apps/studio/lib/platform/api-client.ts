...
let _getWorkspaceId: (() => Promise<string | null>) | null = null;

export function configurePlatformApiToken(fn: () => Promise<string | null>): void {
  _getToken = fn;
}

export function configurePlatformApiWorkspace(
...
async function getAuthHeaders(): Promise<Record<string, string>> {
  const [token, selectedWorkspaceId] = await Promise.all([
    _getToken ? _getToken() : null,
    _getWorkspaceId ? _getWorkspaceId() : null,
  ]);
  const workspaceId = selectedWorkspaceId?.trim() || null;

  if (!workspaceId) {
    throw new Error("Select a valid Studio workspace before making a platform request.");
  }
...
function get<T>(path: string)  { return platformRequest<T>(path, { method: "GET" }); }
function del<T>(path: string)  { return platformRequest<T>(path, { method: "DELETE" }); }
function post<T>(path: string, body?: unknown) {
  return platformRequest<T>(path, { method: "POST",  body: body !== undefined ? JSON.stringify(body) : undefined });
}
function patch<T>(path: string, body?: unknown) {
  return platformRequest<T>(path, { method: "PATCH", body: body !== undefined ? JSON.stringify(body) : undefined });
}
