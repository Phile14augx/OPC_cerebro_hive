  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const workflow = await prisma.workflow.findUnique({
        where: { id },
        include: { versions: { orderBy: { version: 'desc' }, include: { nodes: true, edges: true } } }
      });

      if (!workflow || workflow.workspaceId !== workspaceId) {
        return reply.code(404).send({
          success: false,
          error: { code: 'WORKFLOW_NOT_FOUND', message: 'Workflow not found.', requestId: request.cerebroContext.traceId }
        });
      }
