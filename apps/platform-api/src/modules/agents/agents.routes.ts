// @ts-nocheck
import { AgentRepository, prisma } from "@cerebro/db";
...
export default async function agentRoutes(fastify: FastifyInstance, opts: AgentsRouteOptions) {
  ...
  fastify.post("/", { preHandler: requirePermission("agents:create") }, async (request, reply) => {
    ...
    if (!body.name || !body.modelId || !body.instructions) {
      return reply.code(400).send({
        ...
        error: {
          ...
          message:
            "name, modelId, and instructions are required to create an agent with a runnable version.",
